...
static BE_STATUS be_bridge_init(struct be_oper *be)
{
	be->active = 1;
	be->retry = 3;
	be->tout = 2.0;
...
static BE_STATUS be_bridge_finish(struct be_oper *be)
{
	be->active = 0;
	return BE_OK;
}

static BE_STATUS be_bridge_start(struct be_oper *be, struct client *cli)
{
...
static void be_bridge_read_cb(EV_P, struct ev_io *w, int revents)
{
...
static BE_STATUS be_bridge_send(struct client* cli)
{
	mark_time(cli, WRITE_BE);
	if (cli->fd_be == -1) {
		if (be_bridge_start(&be_bridge_oper, cli) != BE_OK)
			return BE_ERROR;
	}
...
static void be_bridge_write_cb(EV_P, struct ev_io *w, int revents)
{
	struct client *cli = ((struct client*) (((char*)w) - offsetof(struct client, ev_write)));

	ev_io_stop(EV_A_ w);
	if (be_bridge_send(cli) == BE_SENT_REQ) {
		ev_io_init(&cli->ev_read, be_bridge_read_cb, cli->fd_be, EV_READ);
...
struct be_oper be_bridge_oper = {
	.name = "bridge",
	.init = be_bridge_init,
	.finish = be_bridge_finish,
	.read_cb = be_bridge_read_cb,
	.write_cb = be_bridge_write_cb,
	.send = be_bridge_send,
};
